...
// pmp
  ...
  /*def checkNotTakenConsecutive = {
    ...
    for (i <- 0 until DecodeWidth - 1) {
      ...
      when (ibuffer.io.out(i).valid && ibuffer.io.out(i).bits.pd.isBr) {
        when (ibuffer.io.out(i+1).valid) {
    ...
    when (ibuffer.io.out(DecodeWidth - 1).valid && ibuffer.io.out(DecodeWidth - 1).bits.pd.isBr) {
      // last instr is a br, record its info
      prevNotTakenValid := true.B
      prevNotTakenFtqIdx := checkTargetIdx(DecodeWidth - 1)
    }
    when (prevNotTakenValid && ibuffer.io.out(0).valid) {
