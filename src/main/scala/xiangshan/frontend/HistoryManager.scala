...
class HistoryManager(implicit p: Parameters) extends XSModule with HasBPUConst with HasCircularQueuePtrHelper {
  val io = IO(new Bundle {
    ...
    val s2_redirect_dup = Input(Vec(numDup, Bool()))
    val s3_redirect_dup = Input(Vec(numDup, Bool()))
  ...
  val do_redirect_dup = dup_seq(RegNextWithEnable(io.ftq_to_bpu.redirect))
