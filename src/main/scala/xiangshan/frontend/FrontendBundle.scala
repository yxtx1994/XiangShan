...
class IBufferToFtq(implicit p: Parameters) extends XSBundle {
  val update = Valid(new LoopCacheSpecInfo)
}

class FtqToIBuffer(implicit p: Parameters) extends XSBundle {
  val pc = Valid(UInt(VAddrBits.W))
}
