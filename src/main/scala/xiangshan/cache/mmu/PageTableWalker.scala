...
class PTW()(implicit p: Parameters) extends XSModule with HasPtwConst with HasPerfEvents {
  ...
  when(mem_addr_update){
    when(level >= 2.U && !onlyS2xlate && !(find_pte || accessFault)){
      level := levelNext
      when(s2xlate){
        s_hptw_req := false.B
      }.otherwise{
...
//  val pte = io.mem.resp.bits.MergeRespToPte()
  ...
  if (EnableSv48) {
    when (mode === Sv48) {
      ppn := Mux(level === 1.U, ppn_l1, Mux(level === 2.U, ppn_l2, ppn_l3)) //for l1, l2 and l3
      pg_base := MakeGPAddr(hgatp.ppn, getGVpnn(vpn, 3.U)) // for l0
      mem_addr := Mux(level === 3.U, pg_base, p_pte)
    } .otherwise {
      ppn := Mux(level === 1.U, ppn_l1, ppn_l2) //for l1 and l2
      pg_base := MakeGPAddr(hgatp.ppn, getGVpnn(vpn, 2.U)) // for l0
      mem_addr := Mux(level === 2.U, pg_base, p_pte)
    }
  } else {
    ppn := Mux(level === 1.U, ppn_l1, ppn_l2) //for l1 and l2
    pg_base := MakeGPAddr(hgatp.ppn, getGVpnn(vpn, 2.U)) // for l0
