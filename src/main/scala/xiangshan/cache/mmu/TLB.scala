  ...
  with HasPerfEvents
{
  ...
  def handle_nonblock(idx: Int): Unit = {
    ...
    val ptw_already_back = GatedValidRegNext(ptw.resp.fire) && req_s2xlate === RegEnable(ptw.resp.bits, ptw.resp.valid).s2xlate && RegEnable(ptw.resp.bits, ptw.resp.valid).hit(get_pn(req_out(idx).vaddr), io.csr.satp.asid, io.csr.vsatp.asid, io.csr.hgatp.asid, allType = true)
