...
class PtwCache()(implicit p: Parameters) extends XSModule with HasPtwConst with HasPerfEvents {
  ...
  if (EnableSv48) {
    val hitVecT = l3.get.zipWithIndex.map {
        case (e, i) => (e.hit(vpn_search, io.csr_dup(2).satp.asid, io.csr_dup(2).vsatp.asid, io.csr_dup(2).hgatp.asid, s2xlate = h_search =/= noS2xlate)
    ...
    for (i <- 0 until l2tlbParams.l3Size) {
      XSDebug(stageReq.fire, p"[l3] l3(${i.U}) ${l3.get(i)} hit:${l3.get(i).hit(vpn_search, io.csr_dup(2).satp.asid, io.csr_dup(2).vsatp.asid, io.csr_dup(2).hgatp.asid, s2xlate = h_search =/= noS2xlate)}\n")
    }
  ...
  val (l2Hit, l2HitPPN, l2Pre) = {
    ...
    for (i <- 0 until l2tlbParams.l2Size) {
      XSDebug(stageReq.fire, p"[l2] l2(${i.U}) ${l2(i)} hit:${l2(i).hit(vpn_search, io.csr_dup(2).satp.asid, io.csr_dup(2).vsatp.asid, io.csr_dup(2).hgatp.asid, s2xlate = h_search =/= noS2xlate)}\n")
    }
  ...
  val (l0Hit, l0HitData, l0Pre, l0eccError) = {
    ...
    val hVec_delay = RegEnable(hVec_req, stageReq.fire)
    val hitVec_delay = VecInit(data_resp.zip(vVec_delay.asBools).zip(hVec_delay).map { case ((wayData, v), h) =>
      wayData.entries.hit(delay_vpn, io.csr_dup(0).satp.asid, io.csr_dup(0).vsatp.asid, io.csr_dup(0).hgatp.asid, s2xlate = delay_h =/= noS2xlate) && v && (delay_h === h)})
  ...
  check_res.l0.apply(l0Hit, l0Pre, l0HitPPN, l0HitPerm, l0eccError, valid = l0HitValid)
  ...
  io.resp.bits.hit      := (resp_res.l0.hit || resp_res.sp.hit) && !isAllStage
  if (EnableSv48) {
  ...
  io.resp.bits.toTlb.entry.map(_.level.map(_ := Mux(resp_res.l0.hit, 0.U, resp_res.sp.level)))
  io.resp.bits.toTlb.entry.map(_.prefetch := from_pre(stageResp.bits.req_info.source))
  for (i <- 0 until tlbcontiguous) {
    io.resp.bits.toTlb.entry(i).ppn := Mux(resp_res.l0.hit, resp_res.l0.ppn(i)(gvpnLen - 1, sectortlbwidth), resp_res.sp.ppn(gvpnLen - 1, sectortlbwidth))
    io.resp.bits.toTlb.entry(i).ppn_low := Mux(resp_res.l0.hit, resp_res.l0.ppn(i)(sectortlbwidth - 1, 0), resp_res.sp.ppn(sectortlbwidth - 1, 0))
    io.resp.bits.toTlb.entry(i).perm.map(_ := Mux(resp_res.l0.hit, resp_res.l0.perm(i), resp_res.sp.perm))
    io.resp.bits.toTlb.entry(i).v := Mux(resp_res.l0.hit, resp_res.l0.v(i), resp_res.sp.v)
  ...
  io.resp.bits.toTlb.not_super := Mux(resp_res.l0.hit, true.B, false.B)
  ...
  if (EnableSv48) {
    when (!flush_dup(2) && refill.levelOH.l3.get && !memPte(2).isLeaf() && !memPte(2).isPf(refill.level_dup(2)) && !memPte(2).isAf()) {
      ...
      l3h.get(refillIdx) := refill.req_info_dup(2).s2xlate
  ...
  when (!flush_dup(2) && refill.levelOH.l2 && !memPte(2).isLeaf() && !memPte(2).isPf(refill.level_dup(2)) && !memPte(2).isAf()) {
    // val refillIdx = LFSR64()(log2Up(l2tlbParams.l2Size)-1,0) // TODO: may be LRU
    ...
    l2h(refillIdx) := refill.req_info_dup(2).s2xlate
  ...
  when (!flush_dup(1) && refill.levelOH.l1 && !memPte(1).isLeaf() && !memPte(1).isPf(refill.level_dup(1)) && !memPte(1).isAf()) {
    ...
    l1h(refillIdx)(victimWay) := refill.req_info_dup(1).s2xlate
  ...
  when (!flush_dup(0) && refill.levelOH.l0 && !memPte(0).isAf()) {
    ...
    l0h(refillIdx)(victimWay) := refill.req_info_dup(0).s2xlate
  ...
  when (sfence_valid) {
    val l2vmidhit = VecInit(l2vmids.map(_.getOrElse(0.U) === io.csr_dup(2).hgatp.asid)).asUInt
    ...
    when (sfence_dup(0).bits.rs1/*va*/) {
      ...
      when (sfence_dup(0).bits.rs2) {
        // specific leaf of addr && all asid
        spv := spv & ~(sphhit & VecInit(sp.map(_.hit(sfence_vpn, sfence_dup(0).bits.id, sfence_dup(0).bits.id, io.csr_dup(0).hgatp.asid, ignoreAsid = true, s2xlate = io.csr_dup(0).priv.virt))).asUInt)
      } .otherwise {
        // specific leaf of addr && specific asid
        spv := spv & ~(~spg & sphhit & VecInit(sp.map(_.hit(sfence_vpn, sfence_dup(0).bits.id, sfence_dup(0).bits.id, io.csr_dup(0).hgatp.asid, s2xlate = io.csr_dup(0).priv.virt))).asUInt)
      }
    }
  }
  ...
  when (hfencev_valid) {
    ...
    when(sfence_dup(0).bits.rs1) {
      ...
      when(sfence_dup(0).bits.rs2) {
        ...
        spv := spv & ~(~spg & sphhit & VecInit(sp.map(_.hit(hfencev_vpn, sfence_dup(0).bits.id, sfence_dup(0).bits.id, io.csr_dup(0).hgatp.asid, s2xlate = true.B))).asUInt)
      }
    }
  }
