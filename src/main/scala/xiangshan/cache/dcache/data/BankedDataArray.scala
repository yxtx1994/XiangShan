...
class BankedDataArray(implicit p: Parameters) extends AbstractBankedDataArray {
  ...
  (0 until LoadPipelineWidth).map(i => {
    ...
    val r_div_addr = RegEnable(div_addrs(i), io.read(i).fire)  //RegNext(RegNext(div_addrs(i)))
    val r_bank_addr = RegEnable(bank_addrs(i), io.read(i).fire)  //RegNext(RegNext(bank_addrs(i)))
    val r_way_addr = RegEnable(OHToUInt(way_en(i)), io.read(i).fire)  //RegNext(RegNext(OHToUInt(way_en(i))))
    ...
    (0 until VLEN/DCacheSRAMRowBits).map( j =>{
      io.read_resp_delayed(i)(j) := bank_result(r_div_addr)(r_bank_addr(j))
      // error detection
      // normal read ports
