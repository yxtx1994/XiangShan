  ...
  with HasPerfEvents
{
  ...
  for (i <- 0 until LoadQueueReplaySize) {
    ...
    addrNotBlockVec(i) := Mux(strict(i), isAfter(io.stAddrReadySqPtr, blockSqIdx(i)), stAddrReadyVec(blockSqIdx(i).value) || isAfter(io.stAddrReadySqPtr, blockSqIdx(i))) || io.sqEmpty // for better timing
