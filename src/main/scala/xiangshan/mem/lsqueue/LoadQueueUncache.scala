  ...
  with HasMemBlockParameters
{
  ...
  val ncOutValidVecRem = SubVec.getMaskRem(ncOutValidVec, LoadPipelineWidth)
      ...
      when (e.io.mmioSelect) {
        ...
        (0 until LoadPipelineWidth).map { w =>
          ...
          when((i.U === idx) && ncOutValid) {
            ncOut(w).valid := ncOutValid
            ncOut(w).bits := e.io.ncOut.bits
            e.io.ncOut.ready := ncOut(w).ready
          }
        }
