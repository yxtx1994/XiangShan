  ...
  with HasPerfEvents
{
  ...
  val s2_hasNotWritebackedLoad = GatedRegNext(VecInit(s1_preReqs.zip(s1_hasNotWritebackedLoad).map(x => x._1 && x._2)))
  ...
  val allocatedUInt = GatedRegNext(allocated.asUInt)
  ...
  (0 until LoadQueueRARSize).map(i => {
    ...
    when (GatedRegNext((paddrModule.io.releaseMmask.takeRight(1)(0)(i) || bypassMatch) && allocated(i) && release1Cycle.valid)) {
  ...
  val ldLdViolationCount = PopCount(io.query.map(resp => GatedRegNext(resp.s2_alloc) && resp.s3_nuke))
