  ...
  with HasPerfEvents
{
  ...
  val s2_enqIdxOHs = s2_enqIdxs.map(UIntToOH(_))
  val s2_releasePAddrMatch = VecInit((0 until LoadPipelineWidth).map(i => {
  ...
  (0 until LoadQueueRARSize).map(i => {
    val bypassMatch = VecInit((0 until LoadPipelineWidth).map(j => s2_accepts(j) && s2_enqIdxOHs(j)(i) && s2_releasePAddrMatch(j))).asUInt.orR
