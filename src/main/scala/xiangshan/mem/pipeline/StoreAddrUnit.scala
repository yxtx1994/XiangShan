...
class StoreAddrUnit(implicit p: Parameters) extends XSModule with HasDCacheParameters {
  ...
  assert(!s0_valid || !s1_ready, "StoreAddrUnit s0 never stall!")
  // Pipeline
  // --------------------------------------------------------------------------------
  // stage 1
  // --------------------------------------------------------------------------------
  ...
  s3_ready := true.B
  assert(!s3_valid || !(s3_kill || s4_ready), "StoreAddrUnit s3 never stall!")
  ...
  io.stout.valid := s4_valid && !s4_bad_nuke_detected
