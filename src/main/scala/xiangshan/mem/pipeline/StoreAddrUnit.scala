...
class StoreAddrUnit(implicit p: Parameters) extends XSModule with HasDCacheParameters {
  ...
  s1_kill := s1_in.uop.robIdx.needFlush(io.redirect) ||
             s1_in.uop.robIdx.needFlush(RegNext(io.redirect)) || s1_tlb_miss || s1_amo

  s1_ready := true.B
  assert(!s0_valid || !(s1_kill || s2_ready), "StoreAddrUnit s0 never stall!")
  ...
  s2_ready := true.B
  assert(!s1_valid || !(s2_kill || s3_ready), "StoreAddrUnit s1 never stall!")
  ...
  assert(!io.stout.valid || !(io.stout.bits.uop.robIdx.needFlush(io.redirect) || io.stout.ready), "StoreAddrUnit writeback never stall!")
