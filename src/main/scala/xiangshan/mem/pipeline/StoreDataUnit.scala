...
class StoreDataUnit(regOut: Boolean = false)(implicit p: Parameters) extends XSModule {
  ...
  if (regOut) {
    ...
    assert(!s0_valid || !s1_ready, "StoreDataUnit s0 never stall!")
    ...
    val s1_out = Wire(new ExuOutput)
    s1_ready := io.out.ready

    when (s0_fire) { s1_valid := true.B }
    .elsewhen (s1_fire) { s1_valid := false.B }
    assert(!s1_valid || !io.out.ready, "StoreDataUnit s1 never stall!")
    ...
    assert(!s0_valid || !io.out.ready, "StoreDataUnit s0 never stall!")
  }
