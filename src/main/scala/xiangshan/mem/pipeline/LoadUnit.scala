  ...
  with SdtrigExt
{
  ...
  when (!s1_dly_err) {
    ...
    s1_out.uop.exceptionVec(loadPageFault)   := io.tlb.resp.bits.excp(0).pf.ld && s1_vecActive && !s1_tlb_miss && !s1_not_tlb_query
    s1_out.uop.exceptionVec(loadAccessFault) := io.tlb.resp.bits.excp(0).af.ld && s1_vecActive && !s1_tlb_miss && !s1_not_tlb_query
    s1_out.uop.exceptionVec(loadAddrMisaligned) := s1_in.uop.exceptionVec(loadAddrMisaligned) && !s1_not_tlb_query
  } .otherwise {
