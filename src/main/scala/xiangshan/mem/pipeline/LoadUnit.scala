...
//    out.isFirstIssue  := src.isFirstIssue
  ...
  io.dcache.s1_kill_data_read   := s1_kill || s1_dly_err || s1_tlb_miss
  ...
  val s2_safe_wakeup = !s2_out.rep_info.need_rep && !s2_mmio && !s2_exception // don't need to replay and is not a mmio and no exceptions
  ...
  io.ldout.valid       := s3_out.valid && !s3_isvec || s3_mmio.valid
