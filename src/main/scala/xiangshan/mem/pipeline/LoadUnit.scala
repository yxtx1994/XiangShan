  ...
  with HasCircularQueuePtrHelper
{
  ...
  s1_ready := true.B
  assert(!s0_valid || !(s1_kill || s2_ready), "LoadUnit s0 never stall!")
  ...
  s2_ready := true.B
  assert(!s1_valid || !(s2_kill || s3_ready), "LoadUnit s1 never stall!")
  ...
  rollback.bits.level       := Mux(s3_rep_frm_fetch || s3_bad_nuke_detected, RedirectLevel.flush, RedirectLevel.flushAfter)
