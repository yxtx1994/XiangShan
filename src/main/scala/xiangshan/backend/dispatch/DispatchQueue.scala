...
class DispatchQueue(size: Int, enqnum: Int, deqnum: Int)(implicit p: Parameters)
  ...
  for (i <- 0 until size) {
    ...
    when (VecInit(validVec).asUInt.orR && canEnqueue) {
      ...
      validDeq0(i) := FuType.isIntDq0Deq0(Mux1H(validVec, io.enq.req.map(_.bits.fuType))) //alu
      validDeq1(i) := FuType.isIntDq0Deq1(Mux1H(validVec, io.enq.req.map(_.bits.fuType))) //brh
    }
  }
