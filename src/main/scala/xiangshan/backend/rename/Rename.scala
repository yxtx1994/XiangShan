...
class Rename(implicit p: Parameters) extends XSModule with HasCircularQueuePtrHelper with HasPerfEvents {
  ...
  intFreeList.io.doAllocate := fpFreeList.io.canAllocate && dispatchCanAcc || io.rabCommits.isWalk
  fpFreeList.io.doAllocate := intFreeList.io.canAllocate && dispatchCanAcc || io.rabCommits.isWalk

  //           dispatch1 ready ++ float point free list ready ++ int free list ready      ++ not walk
  val canOut = dispatchCanAcc && fpFreeList.io.canAllocate && intFreeList.io.canAllocate && !io.rabCommits.isWalk
...
//    }
  ...
  private val stallForFpFL      = inHeadValid && !io.rabCommits.isWalk &&  dispatchCanAcc &&  intFreeList.io.canAllocate && !fpFreeList.io.canAllocate
  ...
  val renamePerf = Seq(
    ...
    ("rename_stall_cycle_fp      ", inHeadValid && !io.rabCommits.isWalk &&  dispatchCanAcc && intFreeList.io.canAllocate && !fpFreeList.io.canAllocate),
  )
