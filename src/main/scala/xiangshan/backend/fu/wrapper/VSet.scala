...
class VSetRvfWvf(cfg: FuConfig)(implicit p: Parameters) extends VSetBase(cfg) {
  ...
  if (cfg.writeVConfig) io.vtype.get.bits := vsetModule.io.out.vconfig.vtype
  if (cfg.writeVConfig) io.vtype.get.valid := !isReadVl && io.out.valid
  if (cfg.writeVConfig) io.vlIsZero.get := !isReadVl && res.vl === 0.U
  if (cfg.writeVConfig) io.vlIsVlmax.get := !isReadVl && res.vl === vlmax
