...
class Entries(implicit p: Parameters, params: IssueBlockParams) extends XSModule {
  ...
  val resps: Vec[Vec[ValidIO[EntryDeqRespBundle]]] = if(params.isLdAddrIQ) VecInit(io.og0Resp, io.og1Resp, io.memAddrIssueResp.get, io.finalIssueResp.get)
  ...
  if (params.deqFuSame) {
    ...
    io.deq(0).deqEntry := Mux(io.deq(0).othersEntryOldestSel.valid, othersEntryOldest(0), subDeqPolicyEntryVec(0))
    io.deq(1).deqEntry := subDeqPolicyEntryVec(1)
