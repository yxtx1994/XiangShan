  ...
  with HasL1PrefetchSourceParameter
{
  ...
  val dtlb_prefetch = VecInit(Seq.fill(1){
    val tlb_prefetch = Module(new TLBNonBlock(1, 2, pftlbParams)) // SMS
  ...
  if (refillBothTlb) {
    ...
    if (ldtlbParams.outReplace) {
      val replace_ld = Module(new TlbReplace(TlbSubSizeVec(dtlb_ld_idx), ldtlbParams))
      replace_ld.io.apply_sep(dtlb_ld.map(_.replace), ptwio.resp.bits.data.entry.tag)
    }
    if (sttlbParams.outReplace) {
      val replace_st = Module(new TlbReplace(TlbSubSizeVec(dtlb_st_idx), sttlbParams))
    ...
    if (l2ToL1Params.outReplace) {
      val replace_l2 = Module(new TlbReplace(TlbSubSizeVec(dtlb_l2_idx), l2ToL1Params))
      replace_l2.io.apply_sep(dtlb_l2ToL1.map(_.replace), ptwio.resp.bits.data.entry.tag)
    }
  }
