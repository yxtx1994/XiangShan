...
class BackendMemIO(implicit p: Parameters, params: BackendParams) extends XSBundle {
  ...
  // Input
  val writeBack = MixedVec(Seq.fill(params.LduCnt + params.StaCnt )(Flipped(DecoupledIO(new MemExuOutput()))) ++ Seq.fill(params.VlduCnt)(Flipped(DecoupledIO(new MemExuOutput(true)))))
