...
class ExceptionGen(params: BackendParams)(implicit p: Parameters) extends XSModule with HasCircularQueuePtrHelper {
  ...
  val s0_out_bits = wb_bits.zip(wb_valid).map{ case(b, v) => utils.HackedAPI.HackedRegEnable(b, v)}
  ...
  val s1_out_bits = utils.HackedAPI.HackedRegEnable(getOldest(s0_out_valid, s0_out_bits), s1_valid.asUInt.orR)
  val s1_out_valid = RegNext(s1_valid.asUInt.orR)

  val enq_valid = RegNext(in_enq_valid.asUInt.orR && !io.redirect.valid && !io.flush)
  val enq_bits = utils.HackedAPI.HackedRegEnable(ParallelPriorityMux(in_enq_valid, io.enq.map(_.bits)), in_enq_valid.asUInt.orR && !io.redirect.valid && !io.flush)
