...
class RobImp(outer: Rob)(implicit p: Parameters) extends LazyModuleImp(outer)
  ...
  if (env.EnableDifftest) {
    for (i <- 0 until CommitWidth) {
      val difftest = DifftestModule(new DiffInstrCommit, delay = 3)
    ...
    for (i <- 0 until CommitWidth) {
      ...
      val difftest = DifftestModule(new DiffBasicInstrCommit, delay = 3)
      difftest.clock   := clock
      difftest.coreid  := io.hartId
      difftest.index   := i.U
      difftest.valid   := io.commits.commitValid(i) && io.commits.isCommit
      ...
      difftest.setSpecial(
        isFused = CommitType.isFused(commitInfo.commitType),
      )
    }
  }
