  ...
  with HasSoCParameter
{
  ...
  class L2TopIO extends Bundle {
    ...
    // val reset_core = Output(Reset())
  }

  class L2TopImpInlined extends Module with InlineInstance {
    val io = IO(new L2TopIO)
  ...
  class L2TopImp(wrapper: LazyModule) extends LazyModuleImp(wrapper) {
    ...
    val innerModule = Module(new L2TopImpInlined)
