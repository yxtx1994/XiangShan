...
(
  ...
  val intSchdParams = {
    ...
    SchdBlockParams(Seq(
      ...
      IssueBlockParams(Seq(
        ExeUnitParams("IMISC0", Seq(VSetRiWiCfg, I2fCfg, VSetRiWvfCfg, JmpCfg, CsrCfg, FenceCfg), Seq(IntWB(port = 4, 1), VfWB(4, 0)), Seq(Seq(IntRD(5, 1)), Seq(IntRD(3, 1)))),
  ...
  val memSchdParams = {
    ...
    SchdBlockParams(Seq(
      ...
      IssueBlockParams(Seq(
        ExeUnitParams("HYU0", Seq(LduCfg, MouCfg), Seq(IntWB(5, 0), VfWB(5, 0)), Seq(Seq(IntRD(6, 0)))),
        ExeUnitParams("HYU1", Seq(StaCfg), Seq(), Seq(Seq(IntRD(6, 0)))), // fake unit, used to create a new writeback port
      ), numEntries = IssueQueueSize, numEnq = 2),
      IssueBlockParams(Seq(
